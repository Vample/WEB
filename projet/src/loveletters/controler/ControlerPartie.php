...
class ControlerPartie {
  ...
  public function effetPrince($idJoueur){
    ...
    if(!$joueur->protect){
      ...
      $posee = new Posee;
      $posee->idJoueur=$idJoueur;
      $posee->idCarte=$possede->idCarte;
      $posee->save();
      $possede->forceDelete();
  ...
  public function affichageDefausse(){
    DBConnection::getInstance();
    if(!isset($_SESSION)){
      session_start();
    }
    $idManche=Manche::where('idPartie','=',$_SESSION['idPartie'])->max('idManche');
    $defausse = Defausse::where('idManche','=',$idManche)->first();
  ...
  public function defausse($idCarte, $idJoueur=null){
    DBConnection::getInstance();
    if(!isset($_SESSION)){
      session_start();
    }
  ...
  public function pioche($idJoueur,$bypass=false){
    ...
    if($joueur->etat_tour=='pioche' || $bypass){
      ...
      $possede->nbCartes++;
      $possede->save();
      $possede2=Possede::where('idJoueur','=',$idJoueur)
                        ->where('idCarte','!=',$possede->idCarte)
                        ->first();
      if($possede2!=null && ($possede2->idCarte==7 || $possede->idCarte==7) && ($possede2->idCarte==5 || $possede2->idCarte==6 || $possede->idCarte==5 || $possede->idCarte==6)){
